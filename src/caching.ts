// Caching runtime: dispatches on CachingDef.mode.
//   - AutomaticCaching: no body mutation (server-side, e.g. OpenAI).
//   - ExplicitCaching: inline cache_control wrapping (e.g. Anthropic).
//   - ResourceCaching: pre-flight POST to lifecycle.createEndpoint, register
//     the cached resource, set body[lifecycle.referenceField], remove the
//     system_instruction from the main body (e.g. Google cachedContents).
...
async function applyResource(
  ...
  if (!sysInstr) {
    // Nothing cacheable — silently skip, matching Go's behaviour.
    return;
  }
