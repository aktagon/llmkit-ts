...
export async function submitBatch(
  ...
  try {
    ...
    if (bc.inputMode === "FileReferenceInput") {
      ...
      // (files-api / structured output) that buildAuthHeaders does not set — ride
      // it onto the batch CREATE request, else a file-referencing batch item
      // silently drops the beta (batch-modality witness family).
...
// collects them into headersOut per request; the batch submit otherwise sends
// only auth headers).
