...
describe("structured output — schema()", () => {
  const schema = `{"type":"object","properties":{"color":{"type":"string"}},"additionalProperties":false}`;

  test("Google: sibling responseMimeType + responseSchema under generationConfig", async () => {
    let receivedBody: any = {};
    const server = Bun.serve({
      port: 0,
      fetch: async (req) => {
    ...
    try {
      const c = newClient(Providers.google, "key");
      c.provider.baseUrl = `http://localhost:${server.port}`;
      const resp = await c.text.schema(schema).prompt("color of sky");

      expect(resp.text).toBe(`{"color":"blue"}`);
      expect(receivedBody.generationConfig.responseMimeType).toBe(
        "application/json",
      );
      expect(receivedBody.generationConfig.responseSchema.type).toBe("object");
      // Google requires additionalProperties stripped.
      expect(
        receivedBody.generationConfig.responseSchema.additionalProperties,
      ).toBeUndefined();
    } finally {
      server.stop(true);
    }
  ...
  test("OpenAI: wrapped json_schema with strict and auto-required", async () => {
    let receivedBody: any = {};
    const server = Bun.serve({
      port: 0,
      fetch: async (req) => {
    ...
    try {
      ...
      expect(resp.text).toBe(`{"color":"blue"}`);
      expect(receivedBody.response_format.type).toBe("json_schema");
      expect(receivedBody.response_format.json_schema.strict).toBe(true);
      expect(
        receivedBody.response_format.json_schema.schema.additionalProperties,
      ).toBe(false);
      expect(receivedBody.response_format.json_schema.schema.required).toEqual([
        "color",
      ]);
    } finally {
      server.stop(true);
  ...
  test("Anthropic: flat output_format with beta header", async () => {
    let receivedBody: any = {};
    let receivedHeaders: Headers | undefined;
    const server = Bun.serve({
      port: 0,
    ...
    try {
      ...
      expect(resp.text).toBe(`{"color":"blue"}`);
      expect(receivedBody.output_format.type).toBe("json_schema");
      expect(receivedBody.output_format.schema.type).toBe("object");
      expect(receivedHeaders?.get("anthropic-beta")).toBe(
