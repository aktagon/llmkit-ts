...
- `Response.finishReason` and `Response.finishMessage` — provider stop signal + free-text explanation passed through verbatim on `c.text.prompt()`, `c.agent.prompt()`, and `c.text.batch()`. Examples: Anthropic `stop_reason`, OpenAI `choices[0].finish_reason`, Google `candidates[0].finishReason`. Optional fields (`?: string`); empty when the provider response carries no signal. SSE streaming (`c.text.stream()`) does not yet populate them — deferred to a follow-up.
